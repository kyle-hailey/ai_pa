...
- The query contains WHERE predicates like: table.field > value, table.field < value, table.field BETWEEN value1 AND value2, or ORDER BY table.field.
- The table.field has lack a ASC of DESC index 
- If table.field is the primary key (PK) the table is hash-partitioned.
...
When suggesting recreating the table with Range partioning using ASC, keep the primary key PK the same
If table.field is not the primary key PK, don't suggest recreating the table.
when table.filed is not the primary key, only suggest creating secondary indexes
when table.filed is primary key, suggest all 4 solutions
...
Recreate the table with ASC primary key to directly store data ordered by key:
